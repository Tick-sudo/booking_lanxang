...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['confirm']) && $_POST['confirm'] === 'Yes') {
        $sql_delete = "DELETE FROM customer1 WHERE id = ?";
        ...
        // ถ้ากด No หรือไม่ยืนยัน กลับหน้า customers.php
        header("Location: customers.php");
        exit();
    }
}

// ดึงข้อมูลลูกค้าที่ต้องการลบมาแสดง (เพื่อแสดงชื่อ)
$sql = "SELECT * FROM customer1 WHERE id = ?";
...
<head>
    <meta charset="UTF-8" />
    <title>Delete Customer - Admin</title>
...
<!-- Content -->
<div class="content">
    <h2 class="mb-4">ລົບລູກຄ້າ</h2>
    <p>ທ່ານແນ່ໃຈບໍ່ທີ່ຈະລົບລູກຄ້າ<strong><?= htmlspecialchars($customer['Name'] . ' ' . $customer['Lname']) ?></strong>?</p>
